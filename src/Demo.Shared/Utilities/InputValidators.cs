...
public static class InputValidators
{
    ...
    private static readonly SearchValues<char> digitsPlus = SearchValues.Create("+1234567890");
    ...
    public static bool IsAnyMobile(this string? input)
        => !string.IsNullOrWhiteSpace(input) &&
           input.StartsWith('+') && input.Count(x => x == '+') == 1 &&
           input.Trim().Length >= 5 &&
           input.Trim().Length <= 15 &&
           !input.AsSpan().ContainsAnyExcept(digitsPlus);
    ...
    public static bool IsLength(this string? input, int minimum, int? maximum = 0)
    {
        if (maximum == 0) return input?.Length >= minimum;
